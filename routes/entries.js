...
router.post("/edit-entry", async (req, res) => {
    try {
        if (req.session.username) {
            ...
            if (results[0]["COUNT(*)"]) {
                await pool.query(`UPDATE History SET Generations = Generations + 1 WHERE EntryID = ?;
                                  INSERT INTO History (EntryID, Content, PostTime, Generations) SELECT EntryID, Content, PostTime, 1 FROM Entries WHERE EntryID = ?;
                                  UPDATE Entries SET Content = ? WHERE EntryID = ?;`, [entryID, entryID, content, entryID], connection);
                connection.release();
                res.status(200).end();
            } else {
                connection.release();
